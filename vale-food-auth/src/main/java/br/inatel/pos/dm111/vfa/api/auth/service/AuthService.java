...
public class AuthService {
	...
	private String generateToken(User user) {
		...
		return Jwts.builder().issuer(tokenIssuer).subject(user.email()).claim("role", user.type())
				.issuedAt(Date.from(now)).expiration(Date.from(now.plusSeconds(600))).signWith(privateKey).compact();
	}
